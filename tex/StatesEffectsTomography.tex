...
\date{2026\\[2pt]\small Version 1.0.1}
...
\begin{document}
  ...
  \subsection{Dependency on Effective Composition}
    ...
    \cite{Beau2026esub}'s Theorem~4.4 supplies it --- as an already-proved hypothesis on the primitive family,
  ...
  \label{sec:cv}
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.55\textwidth]{fig_cv1}
      \caption{The $7 = 2^3-1$ points reached by cylindrical conditioning on $\Delta(\SA)$, $m=3$
      (Theorem~\ref{thm:cv1}), against the shaded full simplex, reachable only once an explicit
      classical-randomisation postulate is added (Section~\ref{sec:closure}).
  ...
  \label{sec:closure}
    ...
    \begin{remark}[Rank governs the generators, not the closure]
      ...
      Theorem~4.4); it becomes invisible once P1 is added, since a rank-one and a fully correlated $W$ both
  ...
  \label{sec:diagonal}
    ...
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.7\textwidth]{fig_diagonal}
      \caption{A rectangle (reachable by local composition) against the diagonal (unreachable,
      Theorem~\ref{thm:bc4}) on $\SA \times \SB = \{0,1\}^2$. Generated by
  ...
  \label{app:repro}
    ...
    \end{itemize}
    Figures~\ref{fig:cv1} and~\ref{fig:diagonal} (Sections~\ref{sec:cv} and~\ref{sec:diagonal}) are generated
    by \texttt{code/make\_figures.py}, under the pinned environment recorded in
...
bash build.sh
    \end{verbatim}
    The scripts print the full inventories; \texttt{build.sh} regenerates the figures, verifies them against
